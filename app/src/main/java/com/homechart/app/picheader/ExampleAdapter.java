...
public class ExampleAdapter extends FlexibleAdapter<AbstractFlexibleItem> {
	...
	public void showLayoutInfo(boolean scrollToPosition) {
		if (!hasSearchText()) {
			...
			item.setSubtitle(mRecyclerView.getContext().getString(
					R.string.columns,
					String.valueOf(com.flexible.flexibleadapter.utils.Utils.getSpanCount(mRecyclerView.getLayoutManager())))
			);
	...
	/**
	 * This is a customization of the Layout that hosts the ic_default_head when sticky.
