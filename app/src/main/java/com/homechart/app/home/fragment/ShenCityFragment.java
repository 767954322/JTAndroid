...
public class ShenCityFragment extends BaseFragment implements View.OnClickListener, PullToRefreshLayout.OnRefreshListener, ListView.OnItemClickListener {
    ...
    private static ShenCityFragment sixCityFragment;

    public static ShenCityFragment getInstance(int position, Context context) {
        if (sixCityFragment == null) {
            sixCityFragment = new ShenCityFragment(position, context);
        }
        consumerHomeActivity = (ConsumerHomeActivity) context;
        return sixCityFragment;
    }

    private ShenCityFragment(int position, Context context) {
    ...
    public void changeCityData(int city_id) {
        this.position = city_id;
        getData(REFRESH_STATUS);
    }
}
