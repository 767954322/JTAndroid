...
public class SwipeListView extends ListView {
    ...
    private boolean judgeHeaderView(float posX, float posY) {
        // if ic_default_head can swipe
        if (mIsHeaderCanSwipe) {
            return true;
        }
        // ic_default_head cannot swipe
        int selectPos = pointToPosition((int) posX, (int) posY);
        if (selectPos >= 0 && selectPos < getHeaderViewsCount()) {
            // is ic_default_head ,can not swipe
            return false;
        }
        // not ic_default_head can swipe
        return true;
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        float lastX = ev.getX();
        float lastY = ev.getY();
        // test footer and ic_default_head
    ...
    /**
     * 设置 footer and ic_default_head can swipe
     *
     * @param footer
     * @param header
     */
